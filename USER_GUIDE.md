...
cd cmw-api
pip install .            # raw-TCP transport, no external dependencies
pip install '.[visa]'    # add VISA support (GPIB / USB-TMC / VXI-11 / HiSLIP)
```

`pyvisa` is only required for VISA connections and is installed by the `[visa]` extra.
The raw TCP transport (`CMW.via_tcp`) has no external dependencies beyond the Python standard library.

### API namespaces

Each technology is a sub-module on the `CMW` object — `cmw.system`, `cmw.route`, `cmw.gprf`,
`cmw.lte`, `cmw.nr5g`, `cmw.wcdma`, `cmw.bt`, `cmw.wlan`. Call technology methods through their
namespace (e.g. `cmw.wlan.measure_evm()`, `cmw.lte.configure_cell(...)`). Common lifecycle
commands (`initialize`, `reset`, `identify`, `get_all_errors`, `self_check`) are also available
directly on `cmw`.
...
cmw.system.self_test()         # *TST?  → "0" on pass
...
count = cmw.system.get_error_count()
```

### Status registers

```python
cmw.system.get_status_byte()                    # *STB?
cmw.system.get_event_status()                   # *ESR?  (reads and clears)
cmw.system.set_service_request_enable(0x20)     # *SRE — enable SRQ on MAV
cmw.system.set_event_status_enable(0xFF)        # *ESE
```

### Date / time

```python
cmw.system.set_date(2025, 6, 1)
cmw.system.get_date()                  # "2025,6,1"
cmw.system.set_time(9, 30, 0)
cmw.system.get_time()                  # "9,30,0"
```

### Display

```python
cmw.system.set_display_update(False)   # disable for faster remote operation
cmw.system.set_display_update(True)    # re-enable
```

### File system (MMEMory)

```python
cmw.system.mem_catalog()                      # list root directory
cmw.system.mem_catalog("/var/user/tests")     # list specific path
cmw.system.mem_store("/var/user/tests/state1.savrcl")   # save instrument state
cmw.system.mem_load("/var/user/tests/state1.savrcl")    # restore state
cmw.system.mem_delete("/var/user/tests/old.savrcl")
cmw.system.mem_mkdir("/var/user/results")
```
...
```python
cmw.route.gprf_set_rx_connector("RF1C")   # receive path for power measurements
cmw.route.gprf_set_tx_connector("RF1C")   # transmit path for generator
cmw.route.gprf_set_rx_path("RX1")
cmw.route.gprf_set_tx_path("TX1")
```
...
cmw.route.lte_set_scenario("SCELl")   # SCELl = single cell (default)
```
...
cmw.route.nr5g_set_scenario("SCELl")
```
...
```python
cmw.route.set_external_attenuation_rx(0.5, connector="RF1C")   # 0.5 dB cable loss on RX
cmw.route.set_external_attenuation_tx(0.5, connector="RF1C")   # 0.5 dB cable loss on TX
```
...
```python
cmw.gprf.gen_set_frequency(2_450_000_000)   # 2450 MHz
cmw.gprf.gen_set_level(-10.0)               # -10 dBm
cmw.gprf.gen_set_modulation("CW")           # CW | WLAN | BT | LTE ...
cmw.gprf.gen_set_output(True)               # RF output ON
print(cmw.gprf.gen_get_level())             # read back level
cmw.gprf.gen_set_output(False)              # RF output OFF
```

### Power Measurement

```python
cmw.gprf.meas_set_frequency(1_800_000_000)      # tune to 1800 MHz
cmw.gprf.meas_set_filter_bandwidth(20_000_000)  # 20 MHz filter
cmw.gprf.meas_set_trigger_source("IMMediate")   # free-run
cmw.gprf.meas_set_count(10)                     # average 10 samples

result = cmw.gprf.meas_read_power(timeout=10.0)
...
```python
result = cmw.gprf.meas_fetch_power()
```

To abort an ongoing measurement:

```python
cmw.gprf.meas_abort_power()
```

### Spectrum Measurement

```python
cmw.gprf.spec_set_center_frequency(2_400_000_000)   # 2.4 GHz centre
cmw.gprf.spec_set_span(100_000_000)                  # 100 MHz span
cmw.gprf.spec_set_rbw(1_000_000)                     # 1 MHz RBW

result = cmw.gprf.spec_read(timeout=15.0)
...
```python
cmw.lte.set_band(7)
cmw.lte.set_dl_earfcn(2850)
cmw.lte.set_bandwidth(15)
cmw.lte.set_dl_power(-70.0)
cmw.lte.set_cell_id(42)
cmw.lte.set_mcc_mnc("310", "260")
cmw.lte.set_imsi("310260000000001")
cmw.lte.set_authentication("MILENAGE", ki="000102030405060708090A0B0C0D0E0F",
                                    op="63BFA50EE6523365FF14C1F45F88737D")
cmw.lte.set_category(6)                # UE category
```

### Link adaptation (DL/UL)

```python
cmw.lte.set_dl_modulation("QAM64")    # QPSK | QAM16 | QAM64 | QAM256 | AUTO
cmw.lte.set_ul_modulation("AUTO")
cmw.lte.set_dl_rb_count(100)          # allocate all RBs for 20 MHz
cmw.lte.set_ul_rb_count(50)
cmw.lte.set_mimo(2)                   # 2×2 MIMO
```
...
cmw.lte.connect(timeout=60)
print(cmw.lte.get_call_status())    # "CONNected"
...
```python
cmw.lte.initiate_handover(target_band=7, earfcn=2850)
```

### RX quality measurement (RSRP / RSRQ / RSSI / SINR)

```python
rx = cmw.lte.measure_rx_quality(timeout=15.0)
...
```python
rx = cmw.lte.fetch_rx_quality()
```

### Throughput (IP data)

```python
cmw.lte.configure_throughput(
    direction="DLULink",   # DL | UL | DLULink
    duration_s=10.0,
    payload_kb=0,          # 0 = time-based (unlimited payload)
)
result = cmw.lte.measure_throughput(timeout=30.0)
...
```python
pwr = cmw.lte.measure_ul_power(timeout=15.0)
...
```python
result = cmw.lte.measure_bler(samples=1000, timeout=30.0)
...
```python
result = cmw.lte.measure_evm(timeout=15.0)
...
```python
cmw.nr5g.set_mode("NSA")
cmw.nr5g.endc_set_lte_anchor_band(3)
cmw.nr5g.endc_set_lte_earfcn(1300)
cmw.nr5g.cell_on()
cmw.nr5g.expect_attach(timeout=90)
print(cmw.nr5g.endc_get_status())    # "CONNected" when NR leg is up
```

### RX quality (SS-RSRP / SS-RSRQ / SS-SINR / CSI-RSRP)

```python
rx = cmw.nr5g.measure_rx_quality(timeout=15.0)
...
result = cmw.nr5g.measure_throughput()

bler = cmw.nr5g.measure_bler(samples=1000)
evm  = cmw.nr5g.measure_evm()
```
...
```python
cmw.wcdma.cell_on()
cmw.wcdma.expect_attach(timeout=60)
cmw.wcdma.connect(timeout=60)
cmw.wcdma.disconnect()
cmw.wcdma.cell_off()
```
...
```python
cmw.bt.set_frequency(channel=0)          # BT Classic channel 0 = 2402 MHz; range 0–78
cmw.bt.set_expected_power(dbm=4.0)       # approximate DUT TX power (Classic only)
cmw.bt.set_packet_type("DH1")            # DH1 | DH3 | DH5 | 2DH1 | 2DH3 | 3DH1 | 3DH3 | 3DH5
cmw.bt.set_trigger_source("POWer")       # IMMediate | POWer | EXTernal
cmw.bt.set_burst_count(100)              # statistics averaging count
cmw.bt.set_payload_pattern("PRBS9")      # PRBS9 | PRBS15 | 11110000 | 10101010 | ALLOnes | ALLZeros
cmw.bt.set_whitening(True, tech="BT")    # data whitening ON (standard) / OFF (debug)
```

#### TX power

```python
result = cmw.bt.measure_tx_power(timeout=15.0)
...
```python
result = cmw.bt.fetch_tx_power()
```

#### Frequency deviation / modulation

```python
mod = cmw.bt.measure_modulation()
...
```python
result = cmw.bt.measure_icft(timeout=15.0)
...
```python
cmw.bt.set_packet_type("2DH1")   # or 3DH1, 3DH5, etc.
result = cmw.bt.measure_devm(timeout=15.0)
...
```python
result = cmw.bt.measure_frequency_drift(duration_s=1.0, timeout=30.0)
...
result = cmw.bt.measure_ber(timeout=30.0)
...
cmw.bt.le_set_channel(37)               # LE channel index 0–39 (37/38/39 = advertising)
cmw.bt.le_set_expected_power(dbm=0.0)   # expected DUT TX power (LE-specific)
cmw.bt.set_whitening(True, tech="BLE")  # data whitening ON (standard) / OFF (debug)
```
...
```python
pwr = cmw.bt.le_measure_tx_power(timeout=15.0)
...
```python
mod = cmw.bt.le_measure_modulation(timeout=15.0)
...
```python
rssi = cmw.bt.le_measure_rssi(timeout=15.0)
...
```python
result = cmw.bt.le_measure_frequency_offset(timeout=15.0)
...
```python
ber = cmw.bt.le_measure_ber(packet_count=1000, timeout=30.0)
...
```python
result = cmw.bt.le_measure_per(packet_count=1500, timeout=30.0)
...
```python
cmw.bt.le_configure_cte(cte_type="AOA", length_us=16)   # AOA | AOD | CONnectionless
...
cmw.wlan.set_channel(36)                 # 802.11 channel number
cmw.wlan.set_bandwidth(80)               # 20 | 40 | 80 | 160 | 320 MHz
cmw.wlan.set_mcs(11)                     # MCS index — range depends on standard (see table above)
cmw.wlan.set_guard_interval("VSHORT")    # see guard interval table below
cmw.wlan.set_spatial_streams(2)          # 1–4 for N/AC/AX; 1–16 for BE
cmw.wlan.set_expected_power(dbm=20.0)
cmw.wlan.set_trigger_source("POWer")     # IMMediate | POWer | EXTernal
cmw.wlan.set_burst_count(100)
```
...
```python
cmw.wlan.set_standard("G")
cmw.wlan.set_data_rate(54.0)   # 54 Mbps (802.11g max)

cmw.wlan.set_standard("B")
cmw.wlan.set_data_rate(11.0)   # 11 Mbps (802.11b max)
```

### TX power

```python
result = cmw.wlan.measure_tx_power(timeout=15.0)
...
```python
result = cmw.wlan.measure_evm(timeout=15.0)
...
```python
result = cmw.wlan.measure_spectral_mask(timeout=15.0)
...
```python
result = cmw.wlan.measure_spectrum_flatness(timeout=15.0)
...
```python
cmw.wlan.configure_acp(offset_mhz=20.0)      # offset = channel spacing
result = cmw.wlan.measure_acp(timeout=15.0)
...
```python
cmw.wlan.configure_obw(percent=99.0)      # bandwidth containing 99% of power
result = cmw.wlan.measure_obw(timeout=15.0)
...
```python
result = cmw.wlan.measure_rssi(timeout=15.0)
...
```python
cmw.wlan.configure_per(packet_count=1000)
result = cmw.wlan.measure_per(timeout=30.0)
...
```python
cmw.wlan.set_he_ppdu_format("SU")    # SU (single-user) | MU (multi-user DL) | TRIG (triggered UL) | ER (extended range)
```

#### BSS colour (spatial reuse)

```python
cmw.wlan.set_bss_colour(42)   # 0–63; valid for both AX and BE
```
...
cmw.wlan.set_ofdma_ru_allocation("26,26,26,26")
# Single full-channel RU (242 tones = full 20 MHz)
cmw.wlan.set_ofdma_ru_allocation("242")
```

#### Target Wake Time (TWT)

```python
cmw.wlan.set_twt(enabled=True, wake_interval_ms=512.0, sleep_duration_ms=100.0)
```
...
cmw.wlan.set_bandwidth(320)      # up to 320 MHz on 6 GHz
cmw.wlan.set_eht_mcs(13)         # MCS 13 = 4096-QAM 5/6
```
...
cmw.wlan.set_standard("BE")
cmw.wlan.set_bandwidth(160)
cmw.wlan.set_eht_puncturing_pattern("P80")   # puncture an 80 MHz sub-channel first
cmw.wlan.set_eht_mcs(14)                      # BPSK + DCM + DUP robust mode
```

#### Preamble puncturing

```python
cmw.wlan.set_eht_puncturing_pattern("NONE")   # no puncturing (default)
cmw.wlan.set_eht_puncturing_pattern("P80")    # puncture one 80 MHz sub-channel
cmw.wlan.set_eht_puncturing_pattern("P160")   # puncture one 160 MHz sub-channel
```
...
cmw.wlan.set_mlo_link(0, freq_hz=2_437_000_000, bw_mhz=40)    # link 0 — 2.4 GHz ch 6
cmw.wlan.set_mlo_link(1, freq_hz=5_180_000_000, bw_mhz=80)    # link 1 — 5 GHz ch 36
cmw.wlan.set_mlo_link(2, freq_hz=6_135_000_000, bw_mhz=320)   # link 2 — 6 GHz ch 1
cmw.wlan.set_mlo_link_count(3)

# Per-link power results
per_link = cmw.wlan.measure_tx_power_mlo(link_count=3)
for i in range(3):
...
```python
cmw.wlan.set_standard("BE")
cmw.wlan.set_bandwidth(320)
cmw.wlan.set_eht_mcs(11)                   # 1024-QAM
cmw.wlan.set_guard_interval("VSHORT")      # 200 ns
cmw.wlan.set_spatial_streams(4)
cmw.wlan.set_expected_power(dbm=20.0)
...
try:
    result = cmw.lte.expect_attach(timeout=30)
...
try:
    result = cmw.lte.measure_throughput(timeout=20)
...
```python
cmw.lte.set_band(999)          # invalid band
...
with CMW.via_tcp(CMW_IP) as cmw:
    cmw.initialize()
    cmw.route.lte_set_rx_connector("RF1C")
    cmw.lte.configure_cell(BAND_3_20MHZ)
    cmw.lte.cell_on()
    cmw.lte.expect_attach()
    cmw.lte.connect()
    ...
```
...
def run_test(cmw, cfg):
    ...
    cmw.lte.expect_attach()
    cmw.lte.connect()
    return cmw.lte.measure_throughput()
...
```python
cmw.system.set_display_update(False)
# ... run many measurements ...
cmw.system.set_display_update(True)
```
...
cmw.system.mem_store("/var/user/session_start.savrcl")

# Restore if something goes wrong
cmw.system.mem_load("/var/user/session_start.savrcl")
```
...
```python
cmw.gprf.meas_initiate_power()          # fire and move on
# ... do other work here ...
result = cmw.gprf.meas_fetch_power()    # collect when needed
```
...
while time.time() < deadline:
    status = cmw.lte.get_call_status()
